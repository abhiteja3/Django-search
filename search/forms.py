...
class MetamodForm(forms.Form):
     metamodule_name = forms.ModelChoiceField(queryset=Metamodule.objects.distinct(),empty_label='Pick a Meta module')
     ...
     def clean_release_number(self):
       ...
       if metamodule_name and Module.objects.exclude(metamodules__name=metamodule_name).exists():
...
class ReleaseForm(forms.Form):
     ...
     def clean_release_number(self):
       ...
       if release_num and Module.objects.exclude(metamodules__release__number=release_num).exists():
